...
export const submitForm = async (
  ...
  try {
    ...
    if (formData.has_motorcycle) {
      ...
      // Only send manufacture_year if it's actually set      if (formData.manufacture_year) {        formDataToSend.append('manufacture_year', formData.manufacture_year);      }
    }
    ...
    const userData = {
      ...
      category: 'male',
      form_type: 'adult', // Changed from 'man' to match the backend expectations
      employee_id: '',
      department: '',
      role: '',
      job: formData.job || '',
      ...
      // Only include manufacture_year if it has a value to avoid DB errors      ...(formData.manufacture_year ? { manufacture_year: formData.manufacture_year } : {}),
